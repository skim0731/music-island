...
splitIf words = 
    ...
    in 
        (sA, sB, b)
    where
        splitif 0 acc (word:rest)
            | word == "else"    = (reverse acc, rest)
        splitif n acc (word:rest)
            | word == "if"   = splitif (n+1) (word:acc) rest
            | word == "then" = splitif (n-1) (word:acc) rest
            | otherwise      = splitif n     (word:acc) rest
        splitif _ acc []     = (reverse acc, [])
...
eval (".S":words) (istack, cstack, dict, out) = 
    let 
        _out = intercalate " " (reverse (map show istack))
    in
        eval words (istack, cstack, dict, (_out:out))
--eval (".S":words) _ = underflow
...
-- define `swap`
eval ("swap": words) (a:b:istack, cstack, dict, out)
    = eval words (b:a:istack, cstack, dict, out)
...
-- define `rot`
eval ("rot" :words) (a:b:c:istack, cstack, dict, out) = 
    eval words (c:a:b:istack, cstack, dict, out)
...
eval (":":words) (istack, cstack, dict, out) = 
    let
        ...
        _dict = dinsert key (Def val) dict
    in 
        eval rest (istack, cstack, _dict, out)

-- define `<cond> if <true branch> [else <false branch>] then` (hint, use `splitIf`)
eval ("if":words) (i:istack, cstack, dict, out) = 
    let 
        (truebranch, falsebranch, rest) = splitIf words
    in case i of 
        0  -> eval (falsebranch++ rest) (istack, cstack, dict, out)
        _  -> eval (truebranch ++ rest) (istack, cstack, dict, out)
